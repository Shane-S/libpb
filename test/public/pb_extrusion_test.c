...
START_TEST(extrude_floor_ceiling_ceiling_only)
{
    ...
    pb_vert3D expected_points[] = {
            { 0.f,  0.f,  0.f,  0.f, -1.f, 0.f, 0.5f, 0.5f},
            { 0.f,  0.f,  2.5f, 0.f, -1.f, 0.f, 0.5f, 1.f },
            {-2.5f, 0.f,  2.5f, 0.f, -1.f, 0.f, 1.f,  1.f },
            ...
            {-2.5f, 0.f, -2.5f, 0.f, -1.f, 0.f, 1.f,  0.f },
            { 2.5f, 0.f,  0.f,  0.f, -1.f, 0.f, 0.f,  0.5f},
            { 0.f,  0.f,  0.f,  0.f, -1.f, 0.f, 0.5f, 0.5f},
    ...
    pb_shape3D expected_ceiling_shape ={&expected_points[0], expected_point_counts[0],
                                      {-2.5f, 6.f, 2.5f}};
    pb_shape3D* expected_ceiling_shapes[] = {
            &expected_ceiling_shape,
    };
    size_t expected_num_ceiling_shapes = sizeof(expected_ceiling_shapes) / sizeof(pb_shape3D*);
    ...
    ck_assert_msg(num_ceiling_shapes_out == expected_num_ceiling_shapes, "Expected num_ceiling_shapes == %lu, was %lu",
                  expected_num_ceiling_shapes, num_ceiling_shapes_out);
    for (i = 0; i < expected_num_ceiling_shapes; ++i) {
        ck_assert_msg(ceiling_shapes_out[i].num_tris == expected_point_counts[i] / 3,
                      "ceiling shape %lu: expected num_tris == %lu, was %lu",
                      i, expected_point_counts[i], ceiling_shapes_out[i].num_tris);

        ck_assert_msg(assert_close_enough(ceiling_shapes_out[i].pos.x, expected_ceiling_shapes[i]->pos.x, 5) &&
                      assert_close_enough(ceiling_shapes_out[i].pos.y, expected_ceiling_shapes[i]->pos.y, 5) &&
                      assert_close_enough(ceiling_shapes_out[i].pos.z, expected_ceiling_shapes[i]->pos.z, 5),
                      "ceiling shape %ld: expected pos (%.3f, %.3f, %.3f), was (%.3f, %.3f, %.3f)",
                      i, expected_ceiling_shapes[i]->pos.x, expected_ceiling_shapes[i]->pos.y, expected_ceiling_shapes[i]->pos.z,
                      ceiling_shapes_out[i].pos.x, ceiling_shapes_out[i].pos.y, ceiling_shapes_out[i].pos.z);

        pb_vert3D* verts = ceiling_shapes_out[i].tris;
        for (j = 0; j < ceiling_shapes_out[i].num_tris * 3; ++j) {
            pb_vert3D* vert = verts + j;
            pb_vert3D* expected_vert = expected_ceiling_shapes[i]->tris + j;

            /* Back to using assert_close_enough because my other float comparison function chokes in some situations */
            ck_assert_msg(assert_close_enough(vert->x, expected_vert->x, 5) &&
                          assert_close_enough(vert->y, expected_vert->y, 5) &&
                          assert_close_enough(vert->z, expected_vert->z, 5),
                          "ceiling shape %lu vert %lu: expected position (%.3f, %.3f, %.3f), was (%.3f, %.3f, %.3f)",
                          i, j, expected_vert->x, expected_vert->y, expected_vert->z, vert->x, vert->y, vert->z);

            ck_assert_msg(assert_close_enough(vert->nx, expected_vert->nx, 5) &&
                          assert_close_enough(vert->ny, expected_vert->ny, 5) &&
                          assert_close_enough(vert->nz, expected_vert->nz, 5),
                          "ceiling shape %lu vert %lu: expected normal (%.3f, %.3f, %.3f), was (%.3f, %.3f, %.3f)",
                          i, j, expected_vert->nx, expected_vert->ny, expected_vert->nz, vert->nx, vert->ny, vert->nz);

            ck_assert_msg(assert_close_enough(vert->u, expected_vert->u, 5) &&
                          assert_close_enough(vert->v, expected_vert->v, 5),
                          "ceiling shape %lu vert %lu: expected UVs (%.3f, %.3f), was (%.3f, %.3f)",
                          i, j, expected_vert->u, expected_vert->v, vert->u, vert->v);
        }
    }
...
Suite *make_pb_extrusion_suite(void)
{
    ...
    tcase_add_test(tc_extrude_floor_ceiling, extrude_floor_ceiling_ceiling_only);
    return s;
}